        ...
        <div class="card-body">
            <form @submit.prevent="validate">
                ...
                <div class="form-group">
                        <label 
                        class="font-medium-5 d-block text-center"
                        for="numberOfProducedProductOmera"
                         >تعداد محصول
                        </label>
                        <input 
                        type="number" 
                        class="form-control position-relative input-lg" 
                        id="numberOfProducedProductOmera" 
                        v-model="formData.numberOfProducedProductOmera"
                        placeholder="تعداد محصول تولید شده را وارد کنید" 
                        >
                        <div class="form-text text-danger">
                        {{formData.numberOfProducedProductOmeraValidate}}
                        </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div >
                          ...
                          <div class="form-group">
                            <label class="font-medium-5 d-block text-center" for="endOmeraTimeAndDate">تاریخ و ساعت خاتمه کار</label>
                            <date-picker color="#03a9f4" type='datetime' id="endOmeraTimeAndDate" v-model="formData.endOmeraTimeAndDate"></date-picker>
                        <div class="form-text text-danger">
                        {{formData.endOmeraTimeAndDateValidate}}
                        </div>
                          </div>
                    </div>    
...
setup(){
  ...
  const formData = reactive({
        ...
        numberOfProducedProductOmera:'',
        numberOfProducedProductOmeraValidate:'',
        machinsParticipant:'',
    ...
    function validate(){
        ...
        if(formData.numberOfProducedProductOmera === ""){
            formData.numberOfProducedProductOmeraValidate = "تعداد محصول نباید خالی باشد!"
        }
        else{
            formData.numberOfProducedProductOmeraValidate = ''
        }
        ...
        if(formData.endOmeraTimeAndDate === ""){
            formData.endOmeraTimeAndDateValidate = "تاریخ و ساعت خاتمه کار باید مشخص شود!"
        }
        else{
            formData.endOmeraTimeAndDateValidate = ''
        }

        if(formData.orderProduct !== "" && formData.numberOfProducedProductOmera !== "" && formData.machinsParticipant !== "" && formData.staffParticipant !== "" && formData.endProductionReason === "6" && formData.endOmeraTimeAndDate !== ""){
          endOfPO()
        }
    }
